...
async def stats(_, message):
    ...
    stats = f'<b>BOT STATISTICS INFO</b>\n\n'\
            f'Bot uptime : <code>{currentTime}</code>\n'\
            f'Sys uptime : <code>{osUptime}</code>\n'\
            f'CPU usage  : <code>{cpuUsage}%</code>\n'\
            f'RAM usage  : <code>{mem_p}%</code>\n'\
            f'Disk usage : <code>{disk}%</code>\n'\
            f'Disk space : <code>{get_readable_file_size(free)}/{get_readable_file_size(total)}</code>\n\n'\
    ...
    if config_dict['SHOW_LIMITS']:
        ...
        stats += f'<b>BOT LIMITATIONS</b>\n\n'\
                f'Torrent    : <code>{torrent_limit}</code>\n'\
                f'Gdrive     : <code>{gdrive_limit}</code>\n'\
                f'Ytdlp      : <code>{ytdlp_limit}</code>\n'\
                f'Direct     : <code>{direct_limit}</code>\n'\
                f'Leech      : <code>{leech_limit}</code>\n'\
                f'Clone      : <code>{clone_limit}</code>\n'\
                f'Mega       : <code>{mega_limit}</code>\n'\
                f'Total Tasks: <code>{total_task}</code>\n'\
                f'User Tasks : <code>{user_task}</code>\n\n'
...
async def start(_, message):
    ...
    if len(message.command) > 1:
        ...
        return await sendMessage(message, f'Selamat atas Token Baru-nya!\n\n<b>Itu akan kedaluwarsa setelah {time_str}</b>') 
    elif config_dict['DM_MODE']:
        start_string = f'<b>Mirror Pea Masamba!</b>\n\nSekarang kau bisa mengunakannya.\n'
    else:
        start_string = f'<b>Mirror Pea Masamba!</b>\n\nKalian bisa mirror apap saja di bot ini!\n'
...
async def main():
    ...
    LOGGER.info("Bot Sudah Bisa Digunakan!")
